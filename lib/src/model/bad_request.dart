...
class BadRequest {
  ...
  @override
  String toString() =>
      'BadRequest[requestId=$requestId, errorCode=$errorCode, message=$message]';
      ...
      assert(() {
        for (var key in requiredKeys) {
          assert(
            ...
            'Required key "BadRequest[$key]" is missing from JSON.',
          );
          assert(
            json[key] != null,
            'Required key "BadRequest[$key]" has a null value in JSON.',
          );
        }
        return true;
      }());
  ...
  static Map<String, List<BadRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BadRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
