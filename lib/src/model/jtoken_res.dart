...
class JtokenRes {
  ...
  @override
  String toString() =>
      'JtokenRes[token=$token, tokenType=$tokenType, expiresIn=$expiresIn, scope=$scope]';
      ...
      assert(() {
        for (var key in requiredKeys) {
          assert(
            ...
            'Required key "JtokenRes[$key]" is missing from JSON.',
          );
          assert(
            json[key] != null,
            'Required key "JtokenRes[$key]" has a null value in JSON.',
          );
        }
        return true;
      }());
  ...
  static Map<String, List<JtokenRes>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = JtokenRes.listFromJson(
          entry.value,
          growable: growable,
        );
      }
